...
from .ffmpeg_utils import extract_frame, extract_sequence, ffprobe_video, extract_frame_to_memory
...
def _metrics_from_frame_lists(
    ...
    if marker_frames and marker_matcher is not None:
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    if args.resume and raw_seg_json.exists():
        ...
        for ridx, (start_sec, end_sec) in enumerate(ranges):
            _log(f"Fine-scan range {ridx + 1}/{len(ranges)}: {start_sec:.2f}s -> {end_sec:.2f}s")
            ...
            name_paths = extract_sequence(
                ...
                vf_filters=[name_roi.as_crop_filter()],
            )
            _assert_crop_size(name_paths, name_roi, f"fine range {ridx + 1} name")
            dialog_paths = name_paths
            marker_paths = extract_sequence(
                ffmpeg_path=ffmpeg_path,
                video_path=args.video,
                ...
                start_sec=start_sec,
                duration_sec=duration,
                vf_filters=[marker_roi.as_crop_filter()],
            )
            
            # Extract full frames (no ROI crop) for later VLM use
            full_dir_temp = work_dir / f"fine_{ridx:03d}_full_temp"
            full_paths = extract_sequence(
                ffmpeg_path=ffmpeg_path,
                video_path=args.video,
                output_dir=full_dir_temp,
                fps=scan_fps,
                start_sec=start_sec,
                duration_sec=duration,
                vf_filters=None,  # No crop filter - keep full frame
            )
            ...
            if marker_force_threshold is not None:
                ...
                _log(
                    f"Forced marker threshold (range {ridx + 1}): {marker_thr_eff:.3f} "
            ...
            # Filter and save full frames with marker presence above threshold
            full_dir = work_dir / f"fine_{ridx:03d}_full"
            marker_frame_count = 0
            for metric in metrics:
                if metric.marker_presence >= marker_thr_eff and metric.frame_index < len(full_paths):
                    src = full_paths[metric.frame_index]
                    dst = full_dir / src.name
                    dst.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(src, dst)
                    marker_frame_count += 1
            _log(f"Range {ridx + 1} full frames: saved {marker_frame_count} marker frames from {len(full_paths)} total")
            
            # Clean up temporary full frames (if not in debug mode)
            if not debug_mode and full_dir_temp.exists():
                shutil.rmtree(full_dir_temp)
    ...
    for i, raw in enumerate(raw_segments, start=1):
        ...
        if has_name:
            ...
            sample_fids = [anchor_fid]
            seg_key = seg_label.replace(" ", "_")
            seg_t0 = f"{float(raw['time_start']):.3f}".replace(".", "p")
            seg_t1 = f"{float(raw['time_end']):.3f}".replace(".", "p")
            for fid in sample_fids:
                # OCR dialogue strictly from tail-anchor frame inside current segment.
                ts = float(raw["start_sec"]) + (float(fid) / float(raw["scan_fps"]))
                selected_ts_for_ocr = ts
                
                # Try to load full frame from cache first
                ridx = int(raw["range_index"])
                full_cache_dir = work_dir / f"fine_{ridx:03d}_full"
                full_image = _try_load_cached_full_frame(
                    ts,
                    full_cache_dir,
                    float(raw["start_sec"]),
                    float(raw["scan_fps"]),
                ...
                # Fallback to extract from ffmpeg if not in cache
                if full_image is None:
                    ...
                    full_image = Image.open(io.BytesIO(frame_bytes)).convert("RGB")
                
                # Crop both ROIs to base64 directly in memory.
                dialog_base64_url = _crop_image_to_base64(full_image, dialogue_roi)
                speaker_base64_url = _crop_image_to_base64(full_image, name_roi)
        ...
        if (
            ...
            if cached_item:
                ...
                cache_hit_count += 1
                _log(
                    f"[VLM] segment {seg.segment_id}: cache hit, skip request"
                )
