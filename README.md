...
如果文本抽取和翻译共用同一个本地 llama.cpp 服务，可通过 `translation.text_extraction_prefetch_during_translation` 控制是否在翻译当前段时后台预取下一段。默认 `auto` 会让 RapidOCR 并行预取、VLM 抽取不并行；本地 llama.cpp `np=1` 建议保持 `auto` 或 `false`，`np>=2` 或远端 API 可设为 `true`。
...
- **翻译**：`vlm_responses` 会将姓名和对话截图传给 VLM 识别/翻译；`ocr_chat_completions` 会先按配置用 OCR 或 VLM 抽取原文，并在满足上下文窗口后把原文传给 chat-completions 翻译
- **字幕导出**：生成 ASS 字幕文件
...
- **编辑字幕**：修改片段起止时间、翻译文本
- **再次自动review**：对当前 cache 调用 LLM 校对，模型建议不会自动写入；每条修改都需要人工确认后才应用
