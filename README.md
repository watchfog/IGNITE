...
`ocr_chat_completions` 模式会先抽取 `name_roi` / `dialogue_roi` 上的说话人与原文，把结果写入 cache 的 `speaker` / `text_original`，再调用所选 profile 的 `/chat/completions` 接口翻译。文本抽取后端默认是 RapidOCR，也可在 Profile GUI 中切换为 VLM Responses 或新加的 VLM Chat Completions 图像识别后端；抽取模型可以是本地 llama.cpp，也可以是远端 VLM API。

使用 `vlm_responses` 时，所选模型必须带有视觉识别功能且支持 `responses` 模式调用。使用 `ocr_chat_completions` 时，翻译模型只需要支持 chat-completions 文本翻译；如果文本抽取后端选择 VLM，则抽取模型需要支持图像输入。默认配置提供 `local-vlm` 抽取 profile，指向 `http://127.0.0.1:8080/v1`，用于本地兼容服务。

使用 VLM 文本抽取时，pipeline 会先完成全部分段的图像文字识别，再开始按顺序翻译，避免本地 VLM 与翻译请求交错竞争同一个服务。`translation.text_extraction_prefetch_during_translation` 仅影响 RapidOCR 后端：默认 `auto` 会在翻译当前段时后台预取后续原文。

常用文本抽取配置示例：

```yaml
translation:
  mode: ocr_chat_completions
  text_extraction_backend: vlm_chat_completions  # ocr / vlm_responses / vlm_chat_completions
  text_extraction_model_profile: local-vlm
  text_extraction_enable_thinking: false
  text_extraction_prefetch_during_translation: auto
```
...
- **文本抽取后端 / 抽取模型**：`ocr_chat_completions` 下可选择 `ocr`、`vlm_responses` 或 `vlm_chat_completions` 作为原文抽取方式
- **编辑额外要求**：添加自定义翻译提示词
...
- **翻译**：`vlm_responses` 会将姓名和对话截图传给 VLM 识别/翻译；`ocr_chat_completions` 会先按配置用 OCR 或 VLM 抽取原文，VLM 抽取会全量完成后再进入 chat-completions 翻译
- **字幕导出**：生成 ASS 字幕文件
...
- **标题操作**：插入/更新标题，识别并翻译标题画面；识别方式可选自动、翻译API直识别或本地VLM直识别
- **编辑字幕**：修改片段起止时间、翻译文本
- **再次自动review**：对当前 cache 调用 LLM 校对；点击后可先编辑并保存游戏名/额外要求，模型建议不会自动写入，每条修改都需要人工确认后才应用
...
- **重译**：重新截图并复译（针对 ROI 未框选准的情况），或用原文直接重译；每次发起前都会重新读取配置文件
...
- **ROI 显示开关**：可勾选"显示ROI"在画布上叠加所有配置过的 ROI 矩形框，默认关闭
- **内嵌字幕视频生成**：使用 ffmpeg 渲染硬字幕视频，支持 CPU/NVENC/QSV/AMF 编码

![GUI End](docs/images/title_review.png)

对标题功能：通过预览确定标题开始/结束时间，将时间轴拖动到有标题文字的画面，填写翻译信息和识别方式后点击 `识别并翻译Title` 按钮即可。识别方式为 `自动` 时，若配置中的文本抽取后端是 VLM，会用该 VLM 模型直接识别并翻译标题图像；否则使用原有翻译 API 直识别。
...
python -m ignite.pipeline \
    ...
    --text-extraction-backend <ocr|vlm_responses|vlm_chat_completions> \
    --skip-translation
```
...
| `--text-extraction-backend` | `ocr_chat_completions` 的原文抽取后端：`ocr` / `vlm_responses` / `vlm_chat_completions` |
| `--text-extraction-model` | VLM 图像文字抽取使用的 `translation.model_profiles` 名称 |
...
| `--auto-review-chunk-size` | 自动校对每次调用 LLM 的条目数（默认读取配置中的 80，覆盖配置文件中的设置） |
