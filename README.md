...
```bash
python app/cache_browser_review.py --cache [CACHE] --video [VIDEO] --config [CONFIG]
```
...
其中 debug 版本仅包含识别信息（`segment_id`），用于辅助确认当前字幕所在位置，方便在校对工具中定位。

利用 PotPlayer 或类似播放器的主副字幕功能可以快速定位存在问题的字幕位置。
...
```bash
python -m src.auto_gamevideo_subtitles.pipeline \
...
| `--debug` | 启用 debug 产物 |
| `--name-split-mode` | 姓名分割验证模式：`config` / `mask` / `ocr` |
