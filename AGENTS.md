...
IGNITE — 面向游戏剧情视频的字幕流水线 GUI 工具（OCR/VLM 识别与翻译 → 校对 → 导出 ASS）。纯 Python，Tkinter GUI。
...
python -m ignite.pipeline \  # 直接运行 pipeline（无 GUI）
    ...
    --translation-model <profile> --auto-review
...
| `ignite/pipeline.py` | 核心流水线：分段 → OCR/VLM 识别翻译 → 缓存 → 字幕生成 |
...
| `ocr_engines.py` | 156 | OCR 引擎 (`build_ocr_engine`，RapidOCR)；`recognize_array()` 直接接受 numpy 数组跳过磁盘 I/O，`_parse_ocr_output()` 为共享解析方法 |
| `translation_runtime.py` | — | VLM responses 与 chat-completions 翻译客户端 (`VlmResponsesTranslator` / `ChatCompletionsTextTranslator`) + VLM 图像文字抽取器 (`VlmImageTextExtractor`)；chat-completions 支持滚动上下文窗口（前8句原文）提升术语一致性；model profile 级别 temperature/top_p/top_k（不填不传）；OCR 纠错提示词 `"OCR识别可能有误，对无法辨认的词请结合字形和上下文自行判断"`；`上下文参考` 注入 system prompt 而非 user message |
...
| `datatypes.py` | 50 | 数据类：`VideoMeta`、`Roi`、`OcrResult`、`DialogueSegment`（含 `auto_review_reason: str = ""` 字段） |
| `cache_manager.py` | 226 | 翻译缓存 JSON 读写、索引、命中检查 |
| `auto_review.py` | — | LLM 自动校对：`run_auto_review_entries()` 分块调用 chat-completions 翻译校对，解析 JSON 更新，支持重试/修复；`apply_updates_to_cache_entries()` 按 segment_id 应用修改到 cache；reason 存入 `auto_review_reason` 字段（重载 cache 时不继承） |
...
pipeline.py  ← 编排入口，import 所有下层模块
  ...
  ├── translation_runtime.py (VLM responses / chat-completions 客户端，独立)
  ├── auto_review.py        (LLM 自动校对，依赖 translation_runtime + cache_manager)
...
- RapidOCR 当前使用默认 PP-OCRv5 模型，日文/中文等语种在 RapidOCR 模型枚举中都走 `CH`；`RapidOcrEngine.info()` 里 `requested_lang: japan` 但 `target_lang: ch` 是预期现象，不代表 OCR 语言配置错误。
- 不要因为 `target_lang: ch` 就改 `LangRec.CH` / `LangDet.CH` 或 `rapidocr_rec_lang` 相关逻辑；只有在明确要接入自定义日文 OCR 模型文件时，才需要重新设计模型路径/语言选择。
...
- 翻译模型通过 `translation.model_profiles` 配置；每个 profile 独立指定 `model`、`base_url`、可选 `api_key_file` / `api_key`
- `api_key` 为空时不发送 Authorization header，支持本地兼容服务
...
- `translation.auto_review_*` 配置项控制自动校对行为（模型、chunk_size 等）
...
- cache entry 可能包含 `auto_review_reason` 字段（自动校对写入）；`_cache_entry_with_review_metadata` 清理 review 元数据时会一并清除
