...
| `translation_runtime.py` | — | VLM responses 与 chat-completions 翻译客户端 (`VlmResponsesTranslator` / `ChatCompletionsTextTranslator`) + retry/helper 工具；chat-completions 支持滚动上下文窗口（前8句原文）提升术语一致性；model profile 级别 temperature/top_p/top_k（不填不传）；OCR 纠错提示词 `"OCR识别可能有误，对无法辨认的词请结合字形和上下文自行判断"`；`上下文参考` 注入 system prompt 而非 user message |
...
4. **翻译**：`ocr_chat_completions` 模式下使用滑动 OCR lookahead + ThreadPoolExecutor 异步预取（OCR 在等待 API 响应时后台预取下一批帧），严格顺序翻译并附带滚动上下文窗口
...
- `translation.mode` 支持 `vlm_responses`（图片直传 VLM responses）和 `ocr_chat_completions`（先 OCR 写入 cache 原文字段，再走 chat-completions 文本翻译）
